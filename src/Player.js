...
class Player extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (prevProps.channelData !== this.props.channelData) {
      this.setState({ playerStatus: undefined, videoId: undefined });
      this.updateChannelData();
    }
  }
  ...
  onStateChange(e) {
    ...
    this.setState({ playerStatus: statusStr });
    // Force captions off
  ...
  render() {
    ...
    return (
      <div className="overflow-hidden">
        <div
          className={`video-background overflow-hidden transition-opacity duration-1000 ${isUIVisible ? 'opacity-75 hover:opacity-100' : 'opacity-100'}`}
          style={{
            transition: `transform ${CHROMA_EASING_TIMING} ${CHROMA_EASING_BEZIER}, opacity 300ms ease-out`,
            transform: isUIVisible ? `translate(${MAIN_BAR_WIDTH}px, -${BOTTOM_BAR_HEIGHT}px)` : `translate(0, 0)`,
        ...
        >
          <div
            ...
            style={{ paddingRight: isUIVisible && MAIN_BAR_WIDTH, paddingTop: isUIVisible && BOTTOM_BAR_HEIGHT }}
          >
            {playerStatus === 'error' ? (
          ...
          {videoId && (
            <div className={`video-foreground transition-opacity ease-in-out duration-${VIDEO_TRANSITION_MS} ${playerStatus === 'playing' ? 'opacity-100' : 'opacity-0'}`}>
